<template>
  <div class="col-md-12">
    <h4>Member List</h4>
    <button class="btn btn-primary" v-on:click="showFilter = !showFilter">Show Filter</button>
    <button class="btn btn-secondary" v-if="selectedMembers.length" v-on:click="exportMembers()">Export Selected</button>
    <div v-if="showFilter">
      ...
      <div class="form-group">
        <label for="membership_end_date_start">Membership End Date - Start</label>
        <input
          type="text"
          class="form-control"
      ...
      <button class="btn btn-primary" v-on:click="filterMembers()">Filter</button>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Membership End Date", sortable: true, value: "membership_end_date" },
  ...
  methods: {
    ...
    filterMembers() {
      ...
      if (this.filterMembershipType) {
        filteredMembers = filteredMembers.filter(member => member.type === this.filterMembershipType);
      }

      if (this.filterMembershipEndDateStart) {
        filteredMembers = filteredMembers.filter(member => new Date(member.membership_end_date) >= new Date(this.filterMembershipEndDateStart));
      }

      if (this.filterMembershipEndDateEnd) {
        filteredMembers = filteredMembers.filter(member => new Date(member.membership_end_date) <= new Date(this.filterMembershipEndDateEnd));
      }

      if (this.filterQBDateStart) {
        filteredMembers = filteredMembers.filter(member => new Date(member.qb_date) >= new Date(this.filterQBDateStart));
      }

      if (this.filterQBDateEnd) {
        filteredMembers = filteredMembers.filter(member => new Date(member.qb_date) <= new Date(this.filterQBDateEnd));
      }

      const filteredIds = filteredMembers.map(member => member.id);
      this.selectedMembers = this.selectedMembers.filter(member => filteredIds.includes(member.id));
